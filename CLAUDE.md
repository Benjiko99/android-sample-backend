...
- **Serializers** (`app/serializers/*`) are `module_function` modules returning plain hashes with **camelCase string keys** (`"authorId"`, `"likeCount"`) — that is the wire format the Android client expects. Timestamps are `iso8601`. **One projection per resource, deliberately.** A post names its author by `"authorId"` and never embeds the user; a single post sideloads that author under `"included"` exactly as a page does (`PostsService.author_included`), so a client reads one shape everywhere. `PostSerializer` used to also offer a `full` variant that embedded the author, and `UserSerializer` a `minimal` one — both are gone, and for the same reason: a second projection is something a client has to tell apart from the first, and a *trimmed* one is indistinguishable on the wire from a resource whose optional fields are genuinely empty. `CommentSerializer` still embeds its author, which is the one place two projections' worth of divergence has not been worth removing.
...
- Post videos are uploads too, and their metadata is **measured server-side**: `VideoMetadata` (`app/services/video_metadata.rb`) shells out to `ffprobe` on the tempfile before it is attached, reading `duration_seconds` and the pixel `width`/`height` — the resolution the clip is *displayed* at, so a quarter-turn in the container's display matrix (how a phone records portrait video) transposes the stored pair; `VideoThumbnail` (`app/services/video_thumbnail.rb`) then runs `ffmpeg` on the same tempfile to extract a poster frame from the clip's midpoint, fitted inside a `MAX_EDGE` box so it keeps the clip's own aspect ratio, and stored as an attachment (`thumbnail_width`/`thumbnail_height` record its measured dimensions). ffmpeg is asked for a box rather than two edges because the container's `width`/`height` describe the frame *as stored* — a phone's portrait clip is a landscape frame plus a rotation ffmpeg applies on decode — so computing the edges from them squashed every rotated clip into a 16:9 still. ffmpeg is the one system binary the app needs (`apt install ffmpeg`; it is in the Dockerfile and CI). Every failure — no ffmpeg, unreadable container — degrades (duration 0, resolution null, no thumbnail) rather than failing the upload, so a machine without ffmpeg still serves the endpoint. Like `file`/`url`, the thumbnail is an attachment for uploads or an external `thumbnail_url` for seed data (`Video` calls `serves_attached` twice; see `ServesAttachedFile`).
